...
type repository struct {
	storage store.StoreProductInterface
}

// NewRepository crea un nuevo repositorio
func NewRepository(storage store.StoreProductInterface) Repository {
...
func (r *repository) Update(id int, p domain.Product) (domain.Product, error) {
	...
	prodOld, _ := r.storage.Read(id)

	if p.CodeValue != prodOld.CodeValue && r.storage.Exists(p.CodeValue) {
		return domain.Product{}, errors.New("code value already exists")
	}
